...
import com.mr235.volleyimageloaderdemo.BuildConfig;
...
public class DiskLruImageCache implements ImageCache {
	...
	public void putBitmap(String key, Bitmap data) {
		...
		try {
			editor = mDiskCache.edit(strHashCode(key));
			if (editor == null) {
				return;
			}
	...
	public Bitmap getBitmap(String key) {
		...
		try {
			...
			snapshot = mDiskCache.get(strHashCode(key));
			if (snapshot == null) {
				return null;
			}
	...
	private String strHashCode(String str) {
		if (str == null) {
			str = "";
		}
